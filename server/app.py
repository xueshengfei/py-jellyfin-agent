...
5.5. get_seasons — 获取电视剧的所有季信息
   参数: series_keyword(str)
...
- 用户说"xxx有哪些季" / "xxx共几季" → get_seasons(series_keyword="xxx")
...
class CardType(str, Enum):
    """卡片类型，直接透传 Jellyfin 原始类型名，帮助前端决定跳转到哪个详情页。"""
    MOVIE = "movie"
    SERIES = "series"
    EPISODE = "episode"
    VIDEO = "video"
...
_JELLYFIN_TYPE_MAP: dict[str, CardType] = {
    "Movie": CardType.MOVIE,
    "Series": CardType.SERIES,
    "Episode": CardType.EPISODE,
    "Video": CardType.VIDEO,
    "Season": CardType.SEASON,
    "Audio": CardType.AUDIO,
    "MusicAlbum": CardType.MUSICALBUM,
    "MusicArtist": CardType.MUSICARTIST,
    "MusicVideo": CardType.MUSICVIDEO,
    "Book": CardType.BOOK,
    "ComicBook": CardType.COMICBOOK,
}


def _card_type(jellyfin_type: str) -> str:
    """Jellyfin 类型 → 卡片类型枚举值（小写），未知类型默认 movie。"""
    return _JELLYFIN_TYPE_MAP.get(jellyfin_type, CardType.MOVIE).value
